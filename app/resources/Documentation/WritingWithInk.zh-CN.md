...
- **界面与文档**
  - 界面语言：在支持的系统/环境下可切换为简体中文（菜单、对话框等）。文档与补全语言随界面语言自动选择。

- **中英文语法兼容**
  - 编辑器直接支持中英文 ink 语法混写，无需开关。可在脚本中使用中文关键字，与英文语法等价，例如：`主段` ↔ `===`，`子段` ↔ `=`，`跳转` ↔ `->`，`选项` ↔ `*`，`持久` ↔ `+`，`收束` ↔ `-`，`变量` ↔ `VAR`，`临时` ↔ `temp`，`列表` ↔ `LIST`，`常量` ↔ `CONST`，`包含` ↔ `INCLUDE`，`外部` ↔ `EXTERNAL`，`函数` ↔ `function` 等。
...
本教程帮助你在几分钟内用 Inky 写出第一个可玩的小故事，并熟悉界面与中文功能。

### 1. 安装与打开 Inky

- **Windows**：在项目根目录打开 PowerShell，进入 `app` 目录，执行 `npm install`，再执行 `npm start`。
- **已打包版本**：若你下载的是发布包，直接运行 Inky 即可。

首次打开会看到欢迎界面或空白编辑区。

### 2. 新建或打开一个 .ink 文件

- **新建**：菜单 **文件 → 新建**，或直接开始输入（保存时再命名）。
- **打开**：**文件 → 打开**，选择已有的 `.ink` 文件。

### 3. 写第一段：纯文本 + 一个选项

在编辑器中输入下面内容（可直接用英文语法）：
...
- 点击右侧 **播放** 面板，或使用菜单/快捷键运行故事。
- 在播放区会先显示「你好，世界！」，然后出现两个选项；点击任一选项会继续显示对应回复。
- 修改脚本后，再次播放即可看到更新；播放会尽量从你上次选择的位置继续。

### 5. 继续学习

- **文档**：**帮助 → 显示文档** 可打开《使用墨水写作》全文；左侧目录中从「第一部分：基础知识」起按顺序阅读即可系统学习 ink 语法。
- **中文语法**：可用「主段」「子段」「跳转」「选项」「变量」等中文关键字与英文语法混写，编译前会自动转换。
- **错误与补全**：若写错语法，底部问题列表会显示错误位置与说明（界面为中文时会尽量显示中文提示）。输入 `{` 或空格可触发常用语法与函数补全。
