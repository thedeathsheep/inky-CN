...
- **中文语法模式（视图 → 中文语法模式）**
  - 开启后可在 ink 脚本中使用中文关键字，与英文语法等价，可中英混写。例如：`主段` ↔ `===`，`子段` ↔ `=`，`跳转` ↔ `->`，`选项` ↔ `*`，`持久` ↔ `+`，`收束` ↔ `-`，`变量` ↔ `VAR`，`临时` ↔ `temp`，`列表` ↔ `LIST`，`常量` ↔ `CONST`，`包含` ↔ `INCLUDE`，`外部` ↔ `EXTERNAL`，`函数` ↔ `function` 等。
...
- **新建**：菜单 **文件 → 新建**，或直接开始输入（保存时再命名）。建议将工作语言设为中文：**帮助 → 工作语言 → 简体中文**。
...
- **中文语法**：在 **视图 → 中文语法模式** 下，可用「主段」「子段」「跳转」「选项」「变量」等中文关键字，与英文语法混写；编译前会自动转换。
- **错误与补全**：若写错语法，底部问题列表会显示错误位置与说明（中文工作语言下会尽量显示中文提示）。输入 `{` 或空格可触发常用语法与函数补全。
